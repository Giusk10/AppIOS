...
struct AddExpenseView: View {
        ...
        Button(action: saveExpense) {
            ZStack {
                RoundedRectangle(cornerRadius: 16)
                    .fill(canSave ? Color.spendyGradient : LinearGradient(colors: [Color.gray.opacity(0.3), Color.gray.opacity(0.2)], startPoint: .leading, endPoint: .trailing))
                    .shadow(color: canSave ? Color.spendyPrimary.opacity(0.4) : Color.clear, radius: 12, x: 0, y: 6)
    ...
    private var canSave: Bool {
        !description.isEmpty && !amount.isEmpty && (Double(amount.replacingOccurrences(of: ",", with: ".")) ?? 0) > 0
    }
